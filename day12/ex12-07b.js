...
window.addEventListener("scroll", () => {
  ...
  console.log(scrollY);
  /* 스크롤 위치에 따른 텍스트 변경 */
  if (scrollY > 100 && scrollY <= 630) {
    scrollText.textContent = "지금은 1단계입니다.";
  }
  if (scrollY > 630 && scrollY <= 1400) {
  ...
  if (scrollY > 2200 && scrollY <= 3000) {
    scrollText.textContent = "지금은 4단계입니다.";
    t1.classList.remove("turn");
  }
  if (scrollY > 3000) {
    scrollText.textContent = "마지막 단계입니다.";
  }

  if (scrollY > 450 && scrollY < 1600) {
